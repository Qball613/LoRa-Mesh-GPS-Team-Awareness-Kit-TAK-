...
// NVS keys for persistent mesh state
static const char* NVS_MESH_NAMESPACE = "mesh_state";
static const char* NVS_MESH_VERSION_KEY = "version";
static const char* NVS_SEQ_NUMBER_KEY = "seq_num";
static const char* NVS_NEIGHBORS_KEY = "neighbors";
static const char* NVS_ROUTES_KEY = "routes";
static const char* NVS_MESSAGES_KEY = "messages";

// Serialization format version (increment when format changes)
static const uint8_t NVS_FORMAT_VERSION = 1;
...
bool RoutingEngine::updateMeshVersionIfNewer(uint64_t received_version) {
    if (received_version > mesh_version) {
        ...
        // TODO: Trigger sync request to get missing state updates
        // For now, just log that we detected being out of sync
...
void RoutingEngine::loadState() {
    Preferences prefs;
    if (!prefs.begin(NVS_MESH_NAMESPACE, true)) {  // Read-only
        LOG_W("Could not open NVS for reading, starting fresh");
        mesh_version = 0;
        my_sequence_number = 0;
        return;
    }
    ...
    // Load mesh version
    mesh_version = prefs.getULong64(NVS_MESH_VERSION_KEY, 0);
    LOG_I("Loaded mesh version: %llu", mesh_version);
    
    // Load sequence number
    my_sequence_number = prefs.getUInt(NVS_SEQ_NUMBER_KEY, 0);
    LOG_I("Loaded sequence number: %u", my_sequence_number);
    
    // Load neighbors (binary blob)
    size_t neighbor_size = prefs.getBytesLength(NVS_NEIGHBORS_KEY);
    if (neighbor_size > 0) {
        uint8_t* buffer = new uint8_t[neighbor_size];
        prefs.getBytes(NVS_NEIGHBORS_KEY, buffer, neighbor_size);
        
        // Parse neighbors: [count:2][for each: node_id_len:1, node_id, lat:8, lon:8, alt:4, timestamp:8, rssi:1, battery:1]
        size_t pos = 0;
        if (pos + 2 <= neighbor_size) {
            uint16_t count = buffer[pos] | (buffer[pos+1] << 8);
            pos += 2;
            
            for (uint16_t i = 0; i < count && pos < neighbor_size; i++) {
                uint8_t id_len = buffer[pos++];
                if (pos + id_len > neighbor_size) break;
                
                String node_id((const char*)&buffer[pos], id_len);
                pos += id_len;
                
                if (pos + 30 > neighbor_size) break;  // 8+8+4+8+1+1 = 30 bytes
                
                NeighborEntry neighbor;
                neighbor.node_id = node_id;
                memcpy(&neighbor.last_position.latitude, &buffer[pos], 8); pos += 8;
                memcpy(&neighbor.last_position.longitude, &buffer[pos], 8); pos += 8;
                memcpy(&neighbor.last_position.altitude, &buffer[pos], 4); pos += 4;
                memcpy(&neighbor.last_position.timestamp, &buffer[pos], 8); pos += 8;
                neighbor.last_rssi = (int8_t)buffer[pos++];
                neighbor.battery_level = buffer[pos++];
                neighbor.last_seen_time = millis();  // Mark as just seen
                neighbor.is_active = true;
                neighbor.failure_count = 0;
                neighbor.blacklist_until = 0;
                neighbor.total_failures = 0;
                neighbor.sequence_number = 0;
                
                neighbor_table[node_id] = neighbor;
            }
            LOG_I("Loaded %d neighbors from NVS", neighbor_table.size());
        }
        delete[] buffer;
    }
    
    // Load message history (binary blob)
    size_t msg_size = prefs.getBytesLength(NVS_MESSAGES_KEY);
    if (msg_size > 0) {
        uint8_t* buffer = new uint8_t[msg_size];
        prefs.getBytes(NVS_MESSAGES_KEY, buffer, msg_size);
        
        // Parse messages: [count:2][for each: encoded_msg_len:2, encoded_msg_data]
        size_t pos = 0;
        if (pos + 2 <= msg_size) {
            uint16_t count = buffer[pos] | (buffer[pos+1] << 8);
            pos += 2;
            
            for (uint16_t i = 0; i < count && pos < msg_size; i++) {
                if (pos + 2 > msg_size) break;
                uint16_t msg_len = buffer[pos] | (buffer[pos+1] << 8);
                pos += 2;
                
                if (pos + msg_len > msg_size) break;
                
                LoRaMessage msg;
                if (ProtobufHandler::decodeLoRaMessage(&buffer[pos], msg_len, msg)) {
                    message_history.push_back(msg);
                }
                pos += msg_len;
            }
            LOG_I("Loaded %d messages from NVS", message_history.size());
        }
        delete[] buffer;
    }
...
void RoutingEngine::saveState() {
    ...
    Preferences prefs;
    if (!prefs.begin(NVS_MESH_NAMESPACE, false)) {  // Read-write
        LOG_E("Could not open NVS for writing");
        return;
    }
    
    // Save mesh version
    prefs.putULong64(NVS_MESH_VERSION_KEY, mesh_version);
    
    // Save sequence number
    prefs.putUInt(NVS_SEQ_NUMBER_KEY, my_sequence_number);
    
    // Save neighbors (binary blob) - limit to active neighbors
    {
        // Calculate buffer size needed
        size_t buffer_size = 2;  // count
        for (const auto& pair : neighbor_table) {
            if (!pair.second.is_active) continue;
            buffer_size += 1 + pair.first.length() + 30;  // id_len + id + data
        }
        ...
        buffer[pos++] = (count >> 8) & 0xFF;
        
        for (const auto& pair : neighbor_table) {
            if (!pair.second.is_active) continue;
            const NeighborEntry& n = pair.second;
            
            buffer[pos++] = pair.first.length();
            memcpy(&buffer[pos], pair.first.c_str(), pair.first.length());
            pos += pair.first.length();
            
            memcpy(&buffer[pos], &n.last_position.latitude, 8); pos += 8;
            memcpy(&buffer[pos], &n.last_position.longitude, 8); pos += 8;
            memcpy(&buffer[pos], &n.last_position.altitude, 4); pos += 4;
            memcpy(&buffer[pos], &n.last_position.timestamp, 8); pos += 8;
            buffer[pos++] = (uint8_t)n.last_rssi;
            buffer[pos++] = n.battery_level;
        }
        
        prefs.putBytes(NVS_NEIGHBORS_KEY, buffer, pos);
        delete[] buffer;
        LOG_D("Saved %d neighbors to NVS", count);
    }
    
    // Save message history (binary blob)
    {
        // Encode messages
        ...
        for (const auto& msg : message_history) {
            uint8_t temp[MAX_LORA_PAYLOAD_BYTES];
            size_t len = ProtobufHandler::encodeLoRaMessage(msg, temp, sizeof(temp));
            if (len > 0) {
                encoded_msgs.push_back(std::vector<uint8_t>(temp, temp + len));
                total_size += 2 + len;  // len:2 + data
            }
        }
        ...
        uint16_t count = encoded_msgs.size();
        buffer[pos++] = count & 0xFF;
        buffer[pos++] = (count >> 8) & 0xFF;
        
        for (const auto& enc : encoded_msgs) {
            uint16_t len = enc.size();
            buffer[pos++] = len & 0xFF;
            buffer[pos++] = (len >> 8) & 0xFF;
            memcpy(&buffer[pos], enc.data(), len);
            pos += len;
        }
        
        prefs.putBytes(NVS_MESSAGES_KEY, buffer, pos);
        delete[] buffer;
        LOG_D("Saved %d messages to NVS", count);
    }
    
    prefs.end();
    state_dirty = false;
    last_save_time = millis();
    LOG_I("Mesh state saved to NVS");
}
...
void RoutingEngine::processHello(const LoRaMessage& msg, int8_t rssi) {
    ...
    // Check mesh version - if sender has newer version, we're behind
    if (hello.mesh_version > 0) {
        if (updateMeshVersionIfNewer(hello.mesh_version)) {
            LOG_I("Detected newer mesh version from %s - need state sync", node_id.c_str());
            // TODO: Request state sync from this node
        }
    }
...
void RoutingEngine::updateRoute(const String& destination_id, const String& next_hop,
    ...
    LOG_D("Route updated: dest=%s, next_hop=%s, hops=%d",
